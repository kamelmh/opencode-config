...
# Settings path must be provided or discovered from .opencode/state/self-improve/config/
SETTINGS=""
...
if [[ -z "${SETTINGS}" ]]; then
    # Walk up from worktree or CWD to find .opencode/state/self-improve/config/settings.json
    SEARCH_DIR="${WORKTREE_PATH:-$(pwd)}"
    while [[ "${SEARCH_DIR}" != "/" ]]; do
        if [[ -f "${SEARCH_DIR}/.opencode/state/self-improve/config/settings.json" ]]; then
            SETTINGS="${SEARCH_DIR}/.opencode/state/self-improve/config/settings.json"
            break
        fi
