...
```bash
/joc-teams 2:opencode "implement auth module with tests"
...
- Reject unsupported agent types up front. `/joc-teams` only supports **`opencode`**, **`codex`**, and **`gemini`**.
- If the user asks for an unsupported type such as `expert`, explain that `/joc-teams` launches external CLI workers only.
